...
// FUNCIÓN PARA LIMPIAR Y CORREGIR ACENTOS
function corregirAcentos($texto) {
    if ($texto === null || $texto === '') return '';
    
    // Si el texto ya está en UTF-8 correcto, lo dejamos así
    if (mb_check_encoding($texto, 'UTF-8') && !preg_match('/[ÃÁÀÂÄÉÈÊËÍÌÎÏÓÒÔÖÚÙÛÜÑ]/', $texto)) {
        return $texto;
    }
    
    // Mapa de corrección para caracteres mal codificados
    $correcciones = [
        'Ã' => 'Á', 'Ã' => 'É', 'Ã' => 'Í', 'Ã' => 'Ó', 'Ã' => 'Ú',
        'Ã' => 'À', 'Ã' => 'È', 'Ã' => 'Ì', 'Ã' => 'Ò', 'Ã' => 'Ù',
        'Ã' => 'Â', 'Ã' => 'Ê', 'Ã' => 'Î', 'Ã' => 'Ô', 'Ã' => 'Û',
    ...
    foreach ($correcciones as $mal => $bien) {
        $texto = str_replace($mal, $bien, $texto);
    }
    
    // Intentar convertir desde ISO-8859-1 a UTF-8 si sigue teniendo problemas
    if (preg_match('/[ÃÀÁÂÄ]/', $texto)) {
...
// FUNCIÓN PARA CALCULAR HORAS LABORALES - VERSIÓN URGENTE CORREGIDA
function calcularHorasLaborales($fecha_inicio, $hora_inicio, $fecha_fin, $hora_fin, $hora_original_ticket = null) {
    ...
    try {
        // Si la hora es objeto DateTime, convertir a string
        ...
        if ($hora_original_ticket instanceof DateTime) $hora_original_ticket = $hora_original_ticket->format('H:i:s');
        
        // Si la fecha es objeto DateTime, convertir a string
        ...
        // 🔥 CORRECCIÓN URGENTE: Guardar valores originales para debug
        $fecha_inicio_original = $fecha_inicio;
        $hora_inicio_original = $hora_inicio;
        
        // 🔥 CORRECCIÓN: Si es fecha de creación (formato d/m/Y)
        if (is_string($fecha_inicio) && strpos($fecha_inicio, '/') !== false) {
            // SIEMPRE usar la hora original del ticket para fechas de creación
            if (!empty($hora_original_ticket)) {
                $hora_inicio = $hora_original_ticket;
                error_log("✅ Usando hora original: $hora_inicio para fecha $fecha_inicio");
            } else {
                error_log("❌ Fecha creación sin hora original: $fecha_inicio");
            }
        }
        ...
        // Procesar fecha de inicio (convertir d/m/Y a Y-m-d)
        $fecha_inicio_str = is_string($fecha_inicio) ? $fecha_inicio : '';
        if (strpos($fecha_inicio_str, '/') !== false) {
            $parts = explode('/', $fecha_inicio_str);
            if (count($parts) == 3) {
                $fecha_inicio_str = $parts[2] . '-' . $parts[1] . '-' . $parts[0];
                error_log("Fecha inicio convertida: $fecha_inicio_str");
            }
        }
        
        // Procesar fecha de fin (convertir d/m/Y a Y-m-d si es necesario)
        $fecha_fin_str = is_string($fecha_fin) ? $fecha_fin : '';
        if (strpos($fecha_fin_str, '/') !== false) {
            $parts = explode('/', $fecha_fin_str);
            if (count($parts) == 3) {
                $fecha_fin_str = $parts[2] . '-' . $parts[1] . '-' . $parts[0];
            }
        }
        
        // Crear strings completos
        $inicio_completo = $fecha_inicio_str . ' ' . $hora_inicio;
        $fin_completo = $fecha_fin_str . ' ' . $hora_fin;
        
        error_log("🔍 Inicio completo: $inicio_completo");
        error_log("🔍 Fin completo: $fin_completo");
        
        // Crear objetos DateTime
        $inicio = new DateTime($inicio_completo);
        $fin = new DateTime($fin_completo);
        
        if ($fin <= $inicio) {
            error_log("❌ ERROR: Fin menor o igual a inicio");
            return 0;
        }
        
        // HORARIO LABORAL: Lunes a Viernes 8:30 - 17:30
        $hora_inicio_laboral = 8;
        $minuto_inicio_laboral = 30;
        $hora_fin_laboral = 17;
        $minuto_fin_laboral = 30;
        
        $horas_totales = 0;
        $fecha_actual = clone $inicio;
        
        error_log("📅 Iniciando cálculo día por día:");
        ...
        while ($fecha_actual < $fin) {
            ...
            if ($dia_semana <= 5) {
                $inicio_dia = clone $fecha_actual;
                $inicio_dia->setTime($hora_inicio_laboral, $minuto_inicio_laboral, 0);
                
                $fin_dia = clone $fecha_actual;
                $fin_dia->setTime($hora_fin_laboral, $minuto_fin_laboral, 0);
                
                $inicio_efectivo = ($fecha_actual > $inicio_dia) ? clone $fecha_actual : $inicio_dia;
                $fin_efectivo = ($fin < $fin_dia) ? clone $fin : $fin_dia;
                ...
                if ($fin_efectivo > $inicio_efectivo) {
                    ...
                    error_log("   Día $fecha_str: $horas_dia hrs");
                } else {
                    error_log("   Día $fecha_str: 0 hrs (sin solapamiento)");
                }
            } else {
                error_log("   Día $fecha_str: 0 hrs (fin de semana)");
            }
        ...
        error_log("✅ TOTAL HORAS LABORALES: $horas_totales");
        return round($horas_totales, 2);
        
    } catch (Exception $e) {
        error_log("❌ Error en calcularHorasLaborales: " . $e->getMessage());
        return 0;
    }
}

// FUNCIÓN PARA EXPORTAR A EXCEL - CORREGIDA PARA ACENTOS
if (isset($_GET['export']) && $_GET['export'] == 'excel' && $tienePermisosDashboard) {
    ...
    $sqlExport = "SELECT Id_Ticket, Nombre, Correo, Prioridad, Empresa, Asunto, Mensaje, Fecha, Hora, Estatus, PA, FechaTerminado, HoraTerminado, FechaEnProceso, HoraEnProceso FROM [Ticket].[dbo].[T3] WHERE $whereClause ORDER BY CONVERT(DATETIME, Fecha, 103) DESC, Hora DESC";
    $stmtExport = sqlsrv_query($conn, $sqlExport, $params);
    
    // Headers para Excel con codificación correcta
    header('Content-Type: application/vnd.ms-excel; charset=UTF-8');
    header('Content-Disposition: attachment; filename="reporte_tickets_' . date('Y-m-d') . '.xls"');
    header('Pragma: no-cache');
    header('Expires: 0');
    
    // Agregar BOM para UTF-8 (soluciona acentos en Excel)
    echo "\xEF\xBB\xBF";
    ...
    echo "<tr><th colspan='14'>Generado por: " . $nombre . " - " . date('d/m/Y H:i:s') . "</th></tr><tr></tr>";
    ...
    if ($stmtExport) {
        while ($row = sqlsrv_fetch_array($stmtExport, SQLSRV_FETCH_ASSOC)) {
            ...
            echo "<td>" . corregirAcentos($row['Nombre'] ?? 'N/A') . "</td>";
            echo "<td>" . ($row['Correo'] ?? 'N/A') . "</td>";
            echo "<td>" . corregirAcentos($row['Prioridad'] ?? 'N/A') . "</td>";
            echo "<td>" . corregirAcentos($row['Empresa'] ?? 'N/A') . "</td>";
            echo "<td>" . corregirAcentos($row['Asunto'] ?? 'N/A') . "</td>";
            echo "<td>" . corregirAcentos(substr($row['Mensaje'] ?? 'N/A', 0, 100)) . "</td>";
            echo "<td>" . corregirAcentos($row['Estatus'] ?? 'N/A') . "</td>";
            ...
            echo "<td>" . corregirAcentos($row['PA'] ?? 'Sin asignar') . "</td>";
            
            $fechaTerminado = 'N/A';
            $horaTerminado = 'N/A';
            if ($row['FechaTerminado']) {
                if ($row['FechaTerminado'] instanceof DateTime) {
                    $fechaTerminado = $row['FechaTerminado']->format('d/m/Y');
                } else {
                    $dateObj = date_create_from_format('d/m/Y', $row['FechaTerminado']);
                    $fechaTerminado = $dateObj ? $dateObj->format('d/m/Y') : $row['FechaTerminado'];
                }
            }
            if ($row['HoraTerminado']) {
                if ($row['HoraTerminado'] instanceof DateTime) {
                    $horaTerminado = $row['HoraTerminado']->format('H:i');
                } else {
...
if ($tienePermisosDashboard) {
    ...
    if ($stmtTickets) {
        ...
        while ($ticket = sqlsrv_fetch_array($stmtTickets, SQLSRV_FETCH_ASSOC)) {
            ...
            if (($ticket['Estatus'] ?? '') == 'Atendido' && !empty($ticket['FechaTerminado'])) {
                ...
                if (empty($ticket['HoraEnProceso'])) {
                    $hora_inicio = $ticket['Hora'];
                } else {
                    $hora_inicio = $ticket['HoraEnProceso'];
                }
    ...
    if ($stmtAsunto) {
        ...
        while ($row = sqlsrv_fetch_array($stmtAsunto, SQLSRV_FETCH_ASSOC)) {
            if ($contador < 25) { $asuntoData[$row['Asunto']] = $row['cantidad']; $contador++; }
...
<head>
    ...
    <style>
        * {
        ...
        .water-drop {
            position: absolute;
            background: radial-gradient(circle, rgba(59, 130, 246, 0.12) 0%, rgba(96, 165, 250, 0.08) 30%, transparent 70%);
            border-radius: 50%;
        ...
        .dashboard-header {
            ...
            padding: var(--spacing-sm) var(--spacing-lg) var(--spacing-sm) 280px;
            display: flex;
        ...
        /* Contenido Principal */
        .dashboard-container {
            padding: var(--spacing-lg) var(--spacing-lg) var(--spacing-lg) 280px;
            max-width: 100%;
            margin: 0 auto;
            transition: padding 0.4s ease;
        ...
        /* ===== FILTROS MEJORADOS - SIN ICONOS EN SELECTS ===== */
        .filters-container {
            display: grid;
        ...
        /* Estilo para selects - SOLO TEXTO, SIN ICONOS */
        .filter-select {
        ...
        /* ===== TABLA CON COLUMNAS PERFECTAMENTE MARCADAS ===== */
        .table-section {
            background: var(--card-bg);
            backdrop-filter: blur(20px);
            border-radius: var(--border-radius-lg);
        ...
        /* ENCABEZADOS DE TABLA - CLARAMENTE MARCADOS */
        .tickets-table th {
            text-align: left;
        ...
        /* CELDAS DE TABLA - CON BORDES VISIBLES */
        .tickets-table td {
            padding: var(--spacing-sm) var(--spacing-md);
            border: 1px solid var(--glass-border);
            white-space: nowrap;
        ...
        /* FILAS ALTERNATIVAS */
        .tickets-table tbody tr:nth-child(even) td {
        ...
        .no-permission-icon {
            ...
            color: white;
            font-size: 3.5rem;
            box-shadow: 0 20px 30px rgba(245, 158, 11, 0.3);
        }
        ...
        @media (max-width: 1200px) {
            .dashboard-container {
                padding: var(--spacing-md) var(--spacing-md) var(--spacing-md) 270px;
            }
            ...
            <div id="dashboardContent">
                ...
                <form method="GET" class="filters-container animate-fade-in-up">
                    <div class="filter-item">
                        ...
                        <input type="date" class="filter-input filter-date-input" name="fecha_inicio" value="<?php echo $fecha_inicio; ?>">
                    </div>
                    
                    <div class="filter-item">
                        <label class="filter-label">FECHA FIN</label>
                        <input type="date" class="filter-input filter-date-input" name="fecha_fin" value="<?php echo $fecha_fin; ?>">
                    </div>
                    ...
                    <div class="tiempo-promedio-container">
                        <div class="tiempo-promedio-card animate-fade-in-up">
                            ...
                            <div class="tiempo-promedio-subtitle">
                                Basado en <?php echo $ticketsConTiempoCalculado; ?> de <?php echo $totalTickets; ?> tickets
                                <br><small><i class="fas fa-clock"></i> Horario laboral: Lun-Vie, 8:30-17:30</small>
                            </div>
                        </div>
                    ...
                    <div class="chart-card animate-fade-in-up">
                        <div class="chart-header">
                            ...
                            <div class="chart-actions">
                                <button class="chart-action-btn" data-chart="slaPriorityChart"><i class="fas fa-expand"></i></button>
                            </div>
                        </div>
                    ...
                    <div class="charts-grid">
                        ...
                        <div class="chart-card animate-fade-in-up">
                            <div class="chart-header">
                                ...
                                <div class="chart-actions">
                                    <button class="chart-action-btn" data-chart="asuntoChart"><i class="fas fa-expand"></i></button>
                                </div>
                            </div>
                                ...
                                <tbody>
                                    <?php foreach ($ticketsConTiempo as $ticket): 
                                        ...
                                        if ($ticket['HoraTerminado']) {
                                            if ($ticket['HoraTerminado'] instanceof DateTime) {
                                                $horaTerminado = $ticket['HoraTerminado']->format('H:i');
                                            } else {
                                        ...
                                        if ($tiempoEjec !== null && $tiempoEjec !== '') {
                                            ...
                                            if ($horas < 1) { 
                                                ...
                                                $tiempoEjecFormateado = $dias . 'd ' . round($horas_restantes) . 'h'; 
                                            }
    ...
    <script>
        ...
        function createWaterDrops() {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                drop.style.width = (Math.random() * 150 + 80) + 'px'; 
                drop.style.height = drop.style.width;
                drop.style.animationDelay = Math.random() * 10 + 's'; 
                drop.style.animationDuration = (Math.random() * 8 + 10) + 's';
        ...
        logoutAlert.addEventListener('click', (e) => { 
            if (e.target === logoutAlert) logoutAlert.classList.remove('active'); 
        });
        ...
        permissionAlert.addEventListener('click', (e) => { 
            if (e.target === permissionAlert) permissionAlert.classList.remove('active'); 
        });
            ...
            navLinks.forEach(link => {
                link.addEventListener('click', function(e) {
                    ...
                    if (this.classList.contains('disabled')) {
                        ...
                        permissionMessage.textContent = config && config.permissionRequired ? 
                            `El módulo "${config.title}" está restringido para usuarios autorizados.` : 
                    ...
                    if (config.showModule && config.url) {
                        ...
                        moduleIframe.src = '';
                        <?php if ($tienePermisosDashboard): ?> 
                        setTimeout(() => { initialize3DCharts(); }, 100); 
                        <?php endif; ?>
                    }
